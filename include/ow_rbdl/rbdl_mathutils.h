...
#ifndef OW_RBDL_MATHUTILS_H
#define OW_RBDL_MATHUTILS_H

#include <assert.h>
#include <cmath>

#include "ow_rbdl/rbdl_math.h"
 ...
 */
enum OW_RBDL_DLLAPI LinearSolver {
...
extern OW_RBDL_DLLAPI Vector3d Vector3dZero;
extern OW_RBDL_DLLAPI Matrix3d Matrix3dIdentity;
extern OW_RBDL_DLLAPI Matrix3d Matrix3dZero;

OW_RBDL_DLLAPI inline VectorNd VectorFromPtr (unsigned int n, double *ptr) {
  // \todo: use memory mapping operators for Eigen
  VectorNd result (n);
...
OW_RBDL_DLLAPI inline MatrixNd MatrixFromPtr (unsigned int rows, unsigned int cols, double *ptr, bool row_major = true) {
...
OW_RBDL_DLLAPI bool LinSolveGaussElimPivot (MatrixNd A, VectorNd b, VectorNd &x);

// \todo write test 
OW_RBDL_DLLAPI void SpatialMatrixSetSubmatrix(SpatialMatrix &dest, unsigned int row, unsigned int col, const Matrix3d &matrix);

OW_RBDL_DLLAPI bool SpatialMatrixCompareEpsilon (const SpatialMatrix &matrix_a,
    const SpatialMatrix &matrix_b, double epsilon);
OW_RBDL_DLLAPI bool SpatialVectorCompareEpsilon (const SpatialVector &vector_a,
    const SpatialVector &vector_b, double epsilon);

/** \brief Translates the inertia matrix to a new center. */
OW_RBDL_DLLAPI Matrix3d parallel_axis (const Matrix3d &inertia, double mass, const Vector3d &com);
 ...
 */
OW_RBDL_DLLAPI SpatialMatrix Xtrans_mat (const Vector3d &displacement);
 ...
 */
OW_RBDL_DLLAPI SpatialMatrix Xrotz_mat (const double &zrot);
 ...
 */
OW_RBDL_DLLAPI SpatialMatrix Xroty_mat (const double &yrot);
 ...
 */
OW_RBDL_DLLAPI SpatialMatrix Xrotx_mat (const double &xrot);
 ...
 * by ZYX-Euler angles.
 */
OW_RBDL_DLLAPI SpatialMatrix XtransRotZYXEuler (const Vector3d &displacement, const Vector3d &zyx_euler);

OW_RBDL_DLLAPI inline Matrix3d rotx (const double &xrot) {
  double s, c;
  s = sin (xrot);
  c = cos (xrot);
  return Matrix3d (
...
OW_RBDL_DLLAPI inline Matrix3d roty (const double &yrot) {
  double s, c;
  s = sin (yrot);
  c = cos (yrot);
  return Matrix3d (
...
OW_RBDL_DLLAPI inline Matrix3d rotz (const double &zrot) {
  double s, c;
  s = sin (zrot);
  c = cos (zrot);
  return Matrix3d (
...
OW_RBDL_DLLAPI inline Matrix3d rotxdot (const double &x, const double &xdot) {
  double s, c;
  s = sin (x);
  c = cos (x);
  return Matrix3d (
...
OW_RBDL_DLLAPI inline Matrix3d rotydot (const double &y, const double &ydot) {
  double s, c;
  s = sin (y);
  c = cos (y);
  return Matrix3d (
...
OW_RBDL_DLLAPI inline Matrix3d rotzdot (const double &z, const double &zdot) {
  double s, c;
  s = sin (z);
  c = cos (z);
  return Matrix3d (
...
OW_RBDL_DLLAPI inline Vector3d angular_velocity_from_angle_rates (const Vector3d &zyx_angles, const Vector3d &zyx_angle_rates) {
...
OW_RBDL_DLLAPI inline Vector3d global_angular_velocity_from_rates (const Vector3d &zyx_angles, const Vector3d &zyx_rates) {
...
OW_RBDL_DLLAPI inline Vector3d angular_acceleration_from_angle_rates (const Vector3d &zyx_angles, const Vector3d &zyx_angle_rates, const Vector3d &zyx_angle_rates_dot) {
  double sy = sin(zyx_angles[1]);
  double cy = cos(zyx_angles[1]);
  double sx = sin(zyx_angles[2]);
  double cx = cos(zyx_angles[2]);
