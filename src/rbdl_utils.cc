...
string get_body_name (const ow_rbdl::Model &model, unsigned int body_id) {
...
OW_RBDL_DLLAPI std::string GetModelDOFOverview (const Model &model) {
  stringstream result ("");
...
std::string print_hierarchy (const ow_rbdl::Model &model, unsigned int body_index = 0, int indent = 0) {
  stringstream result ("");
...
OW_RBDL_DLLAPI std::string GetModelHierarchy (const Model &model) {
  stringstream result ("");
...
OW_RBDL_DLLAPI std::string GetNamedBodyOriginsOverview (Model &model) {
  stringstream result ("");
...
OW_RBDL_DLLAPI void CalcCenterOfMass (
  Model &model,
  ...
  for (size_t i = model.mBodies.size() - 1; i > 0; i--) {
    ...
    if (lambda != 0) {
      ...
      model.hc[lambda] = model.hc[lambda]
          + model.X_lambda[i].applyTranspose (model.hc[i]);
    } else {
  ...
  if (com_acceleration) {
    *com_acceleration = Vector3d (
          hdot_tot[3] / mass, hdot_tot[4] / mass, hdot_tot[5] / mass);
  }
...
OW_RBDL_DLLAPI void CalcZeroMomentPoint (
  Model &model,
  const Math::VectorNd &q,
  const Math::VectorNd &qdot,
  const Math::VectorNd &qddot,
...
OW_RBDL_DLLAPI double CalcPotentialEnergy (
    Model &model, 
    const Math::VectorNd &q, 
    bool update_kinematics) {
  double mass;
...
OW_RBDL_DLLAPI double CalcKineticEnergy (
    Model &model, 
    const Math::VectorNd &q, 
    const Math::VectorNd &qdot, 
    bool update_kinematics) {
