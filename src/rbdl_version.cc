...
OW_RBDL_DLLAPI int rbdl_get_api_version() {
  static int compile_version = OW_RBDL_API_VERSION;
  return compile_version;
}

OW_RBDL_DLLAPI void rbdl_check_api_version(int version) {
...
OW_RBDL_DLLAPI void rbdl_print_version() {
  int compile_version = rbdl_get_api_version();
  ...
  if (std::string("unknown") != OW_RBDL_BUILD_REVISION) {
    std::cout << "  revision     : " << OW_RBDL_BUILD_REVISION 
      << " (branch: " << OW_RBDL_BUILD_BRANCH << ")" << std::endl
      << "  build type   : " << OW_RBDL_BUILD_TYPE << std::endl;
  }

#ifdef OW_RBDL_ENABLE_LOGGING
...
#ifdef OW_RBDL_BUILD_ADDON_LUAMODEL
  std::cout << "  LuaModel     : on" << std::endl;
#else
  std::cout << "  LuaModel     : off" << std::endl;
#endif
#ifdef OW_RBDL_BUILD_ADDON_URDFREADER
  std::cout << "  URDFReader   : on" << std::endl;
#else
  std::cout << "  URDFReader   : off" << std::endl;
#endif

  std::string build_revision (OW_RBDL_BUILD_REVISION);
  if (build_revision == "unknown") {
    std::cout << std::endl << "Version information incomplete: to enable version information re-build" << std::endl << "library from valid repository and enable OW_RBDL_STORE_VERSION." << std::endl;
  }
}
}
